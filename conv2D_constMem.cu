...
__global__ void conv_2d(float *N, float *P) {
        ...
        for (int r = 0; r < (2 * R + 1); r++) {
            for (int c = 0; c < (2 * R + 1); c++) {
                ...
                if (inCol >= 0 && inCol < WIDTH && inRow >= 0 && inRow < HEIGHT) {
                    pValue += N[inRow * WIDTH + inCol] * F[r][c];
                }
            }
        }
...
int main() {
    ...
    cudaCheckError(cudaMemcpyToSymbol(F, F_h, filter_width * filter_width * sizeof(float)));
    ...
    // Launch the kernel
    conv_2d<<<dimGrid, dimBlock>>>(N_d, P_d);
